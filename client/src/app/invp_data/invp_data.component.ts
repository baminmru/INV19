...
export class invp_dataComponent implements OnInit {
    ...
    refreshinvp_data() {
        ...
        console.log("clear selection for invp_data on refresh");
        this.AppService.pushSelectedinvp_data(this.currentinvp_data);
    }
    ...
    onSelect(item: invi.invp_data) {
        ...
        this.AppService.pushSelectedinvp_data(item);
    }
    ...
    onConfirmDeletion() {
        this.invp_data_Service.delete_invp_dataById(this.currentinvp_data.invp_dataId).subscribe(data => {this.refreshinvp_data(); this.backToList();}, error => { this.ShowError(error.message); });
    }
            ...
            switch (this.mode) {
                case MODE_NEW: {
                    this.invp_data_Service.create_invp_data(item)
                        .subscribe(data =>{ this.refreshinvp_data();this.backToList();}, error => { this.ShowError(error.message); });
                    break;
                }
                case MODE_EDIT: {
...
/* fill data to array */
        for(var i = 0; i < this.invp_dataArray.length; ++i) {
            ...
            aoa[i+1][1]=this.invp_dataArray[i].groupid_name;
            aoa[i+1][2]=this.invp_dataArray[i].subgroupid_name;
            aoa[i+1][3]=this.invp_dataArray[i].departmentid_name;
            aoa[i+1][4]=this.invp_dataArray[i].machineid_name;
        }
		/* generate worksheet */
...
,            {wch: 50}
    ...
    backToList() {
        ...
        this.mode = MODE_LIST;
        this.currentinvp_data = {} as invi.invp_data;
        console.log("clear selection for invp_data");
        this.AppService.pushSelectedinvp_data(this.currentinvp_data);
    }
}
